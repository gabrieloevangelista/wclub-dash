...
import { 
  Heart, Bookmark, Send, Trash2, Image, 
  Film, X, Clock, Plus, Users
} from 'lucide-react';
...
export default function FeedPage() {
  ...
  const fetchFeedData = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/db?collection=posts');
      if (res.ok) {
  ...
  return (
    <div className="feed-layout fade-in">
      ...
      {/* Stories/Status horizontal bubble reels */}
      ...
      {/* Media Lightbox with Comments Panel */}
      {activeLightbox && (
